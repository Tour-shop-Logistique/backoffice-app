...
import LivraisonCommuneForm from "../components/common/LivraisonCommuneForm";
...
const LivraisonCommuneRates = () => {
    const dispatch = useDispatch();
    const canCreate = useHasPermission('tarification_livraison_commune.create');
    const canEdit = useHasPermission('tarification_livraison_commune.edit');
    const canDelete = useHasPermission('tarification_livraison_commune.delete');
    const canToggleStatus = useHasPermission('tarification_livraison_commune.toggle_status');
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [isRefreshing, setIsRefreshing] = useState(false);

    const { livraisonCommuneTarifs: tarifs, isLoadingLivraisonCommune: isLoading, hasLoaded: hasLoadedTarifs } = useSelector((state) => state.tarification);
    ...
    useEffect(() => {
        if (!hasLoadedTarifs && !isLoading) {
            dispatch(fetchLivraisonCommuneTarifs());
        }
    ...
    const handleRefresh = async () => {
        ...
        try {
            await dispatch(fetchLivraisonCommuneTarifs({ silent: true })).unwrap();
    ...
    const handleAddTarif = async (tarifData) => {
        ...
        try {
            await dispatch(addLivraisonCommuneTarif(tarifData)).unwrap();
            setIsModalOpen(false);
            dispatch(showNotification({ type: 'success', message: 'Tarif de livraison ajouté avec succès.' }));
            dispatch(fetchLivraisonCommuneTarifs({ silent: true }));
        } catch (error) {
    ...
    const handleEditTarif = async (tarifData) => {
        ...
        try {
            ...
            if (id) {
                await dispatch(editLivraisonCommuneTarif({ tarifId: id, tarifData: { montant } })).unwrap();
            }
    ...
    const handleDeleteTarif = async () => {
        ...
        setIsDeleting(true);
        try {
            await dispatch(deleteLivraisonCommuneTarif(tarifToDelete.id)).unwrap();
    ...
    const handleStatusChange = async (tarif) => {
        try {
            ...
            await dispatch(updateLivraisonCommuneTarifStatus(tarif.id)).unwrap();
        } catch (error) {
    ...
    return (
        <div className="space-y-4 pb-6 md:space-y-6 md:pb-12">
            <div className="sticky top-[-24px] md:top-[-32px] z-30 bg-[#f1f5f9] -mx-6 px-6 py-3 md:-mx-8 md:px-8 space-y-4 pt-4 lg:pt-2 pb-3">
                <header className="space-y-3 md:space-y-0 text-black">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-xl md:text-2xl font-bold text-slate-900 tracking-tight">Livraison à domicile</h1>
                            <p className="text-sm md:text-base text-slate-500 mt-0.5 font-medium">
                                Tarif fixe par commune, dernier segment des expéditions interville
                            </p>
                        </div>
            ...
            <Modal
                ...
                subtitle="Montant fixe pour la livraison à domicile dans cette commune"
                size="lg"
                confirmFormId="add-livraison-commune-form"
                isLoading={isSubmitting}
                confirmLabel="Enregistrer"
            >
                <LivraisonCommuneForm id="add-livraison-commune-form" onSubmit={handleAddTarif} communes={communes || []} />
            </Modal>

            <Modal
                isOpen={isEditingModalOpen}
                onClose={() => setIsEditingModalOpen(false)}
                title="Modifier le Tarif de Livraison"
                subtitle={`Commune : ${selectedTarif?.commune?.nom || '?'}`}
                size="lg"
                confirmFormId="edit-livraison-commune-form"
                isLoading={isSubmitting}
                confirmLabel="Mettre à jour"
            >
                {selectedTarif && (
                    <LivraisonCommuneForm id="edit-livraison-commune-form" initialData={selectedTarif} onSubmit={handleEditTarif} communes={communes || []} />
                )}
            </Modal>
...
export default LivraisonCommuneRates;
